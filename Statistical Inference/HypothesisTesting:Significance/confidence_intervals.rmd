...
```{r set-seed}
set.seed(2023)                 # make sure to change the seed
```
...
Bsed on this sample, what can we infer about the population? Based only on this single 
...
We know how to find the mean and standard deviation of a sample in R. And we know that the sample size is 60. 
So the only remaining item is finding the appropriate critical value for a given confidence level. We can
...
correspond to the 97.5th percentile. (1-0.05/2)
...
ifelse(1322.04 < params$mu & 1513.193> params$mu, "yes", "no")
```
...
z_star_99 <- abs(qnorm(1-(1-0.99)/2))
z_star_99
```
...
ci_99 <- ames %>%
           rep_sample_n(size = n, reps = 50, replace = TRUE) %>%
           summarise(lower = mean(area) - z_star_99*(sd(area)/
                                                       sqrt(n)),
                     upper = mean(area) + z_star_99*(sd(area)/
                                                       sqrt(n)))
# ci
ci_99 <- ci_99 %>%
  mutate(capture_mu = ifelse(lower < params$mu & upper > params$mu, "yes", "no"))
...
ggplot(data = ci_data_99, aes(x = ci_bounds, y = ci_id,
                              group = ci_id, color = capture_mu )) +
  geom_point(size = 2) + #start point and end point
  geom_line() + #add line btw point
  geom_vline(xintercept = params$mu, color = "darkgray") # mean 
```
