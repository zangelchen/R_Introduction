...
library(dplyr)
library(ggplot2)
library("car") #for enhanced scatterplot()
```
...
```{r runs-vs-at_bats}
plot(mlb11$runs,mlb11$at_bats, main="Relationship between Runs and Bats", xlab="Runs", ylab="Bats")

#add regression line
abline(lm(at_bats~runs, data=mlb11), col="red")
```

```{r enhanced-runs-vs-at_bats}
scatterplot(at_bats~runs, data=mlb11, 
            smooth=FALSE, legend=list(coords="topleft"), grid=FALSE, id= list(method="mahal",n=nrow(mlb11)),
            xlab="Runs", ylab="Bats",
...
```{r homeruns-vs-runs}
plot(mlb11$runs ~ mlb11$homeruns, main="Relationship between Homeruns and Runs", xlab="Home Runs", ylab="Runs")
h1 <- lm(runs ~ homeruns, data = mlb11)
summary(h1)
...
```{r residual}
y_expected=-2789.2429+0.6305*5579
y_observed=713
...
```{r pick-model}
mlb11%>%
  summarise(bat=cor(runs,at_bats), hit=cor(runs,hits), win=cor(runs,wins), bat_avg=cor(runs,bat_avg))
```
...
```{r runs-vs-new-vars}
mlb11%>%
  summarise(obs=cor(runs,new_obs), slug=cor(runs,new_slug), onbase=cor(runs,new_onbase))
```
...
```{r diag}
plot(mlb11$runs ~ mlb11$new_obs, main="Relationship between On-Base+Slugging and Runs", xlab="On-Base+Slugging", ylab="Runs")
on_base_output <- lm(runs ~ new_obs, data = mlb11)
summary(on_base_output)
```
