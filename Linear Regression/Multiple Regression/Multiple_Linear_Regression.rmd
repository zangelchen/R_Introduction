...
fbeauty_all<-data.frame(evals$age, evals$bty_f1lower, evals$bty_f1upper, evals$bty_f2upper)

plot(fbeauty_all$evals.age, fbeauty_all$evals.bty_f1lower)
points(fbeauty_all$evals.age, fbeauty_all$evals.bty_f1upper, col="green")
points(fbeauty_all$evals.age, fbeauty_all$evals.bty_f2upper, col="red")
```
...
hist(bty_lm$residuals, n=20)
qqnorm(bty_lm$residuals)
qqline(bty_lm$residuals)

#check for const variability of residuals
plot(bty_lm$residuals ~ evals$bty_avg)
plot(abs(bty_lm$residuals) ~ evals$bty_avg)
abline(h = 0)
...
#bty_avg is still significant, addition of gender has increased its significant
```
...
```{r p-val-select}
m_full_noEthnicity <- lm(score ~ rank + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_full_noEthnicity)
```
...
```{r adj-rsq-select}
m_no_bty_avg = lm(score ~ rank + gender + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits, data = evals)
summary(m_no_bty_avg)$adj.r.squared

m_no_clsProfs = lm(score ~ rank + gender + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_credits + bty_avg, data = evals)
summary(m_no_clsProfs)$adj.r.squared

m_no_clsStudent = lm(score ~ rank + gender + language + age + cls_perc_eval + 
    cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m_no_clsStudent)$adj.r.squared

m_no_rank = lm(score ~ gender + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
